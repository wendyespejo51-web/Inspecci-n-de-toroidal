...
inputNodoFinal.addEventListener("input", function () {
  const valor = inputNodoFinal.value.trim().toUpperCase(); // Mayúsculas, sin espacios
  ...
  const filtrados = listaNodofinal.filter(Nodofinal => Nodofinal.includes(valor));

  if (filtrados.length > 0) {
    suggestions2List.style.display = "block";
    filtrados.forEach(Nodofinal => {
      const li = document.createElement("li");
